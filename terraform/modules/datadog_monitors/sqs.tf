resource "datadog_monitor" "sqs_dlq_messages_visible" {
  ...
  query = "sum(last_5m):sum:aws.sqs.approximate_number_of_messages_visible{application:${var.app},environment:${var.env},queuename:*dlq*} by {queuename} > ${var.monitor_config.sqs.dlq_message_threshold}"
...
resource "datadog_monitor" "sqs_message_age" {
  ...
  query = "avg(last_10m):avg:aws.sqs.approximate_age_of_oldest_message{application:${var.app},environment:${var.env}} by {queuename} > ${var.monitor_config.sqs.max_message_age_seconds}"
