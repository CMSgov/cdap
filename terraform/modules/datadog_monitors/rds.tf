resource "datadog_monitor" "rds_cpu_high" {
  ...
  query = "avg(last_10m):avg:aws.rds.cpuutilization{app:${var.app},env:${var.env}} by {dbclusteridentifier} > ${var.monitor_config.rds.cpu_threshold}"
...
resource "datadog_monitor" "rds_freeable_memory_low" {
  ...
  query = "avg(last_10m):avg:aws.rds.freeable_memory{app:${var.app},env:${var.env}} by {dbclusteridentifier} < ${var.monitor_config.rds.freeable_memory_threshold_mb * 1000000}"
...
resource "datadog_monitor" "rds_db_connections_high" {
  ...
  query = "avg(last_10m):avg:aws.rds.database_connections{app:${var.app},env:${var.env}} by {dbinstanceidentifier} > ${var.monitor_config.rds.db_connections_threshold}"
...
resource "datadog_monitor" "rds_replica_lag_high" {
  ...
  query = "avg(last_5m):avg:aws.rds.aurora_replica_lag{app:${var.app},env:${var.env}} by {dbinstanceidentifier} > ${var.monitor_config.rds.replica_lag_seconds * 1000}"
...
resource "datadog_monitor" "rds_deadlocks" {
  ...
  query = "sum(last_5m):sum:aws.rds.deadlocks{app:${var.app},env:${var.env}} by {dbinstanceidentifier} > ${var.monitor_config.rds.deadlock_threshold}"
