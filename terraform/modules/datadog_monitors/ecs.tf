resource "datadog_monitor" "ecs_cpu_high" {
  ...
  query = "avg(last_10m):avg:aws.ecs.service.cpuutilization{app:${var.app},environment:${var.env}} by {servicename} > ${var.monitor_config.ecs.cpu_threshold}"
...
resource "datadog_monitor" "ecs_memory_high" {
  ...
  query = "avg(last_10m):avg:aws.ecs.service.memory_utilization{app:${var.app},environment:${var.env}} by {servicename} > ${var.monitor_config.ecs.memory_threshold}"
