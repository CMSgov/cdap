...
| <a name="input_custom_widgets"></a> [custom\_widgets](#input\_custom\_widgets) | List of custom widgets to render in the dashboard group | <pre>list(object({<br/>    type         = string<br/>    title        = string<br/>    display_type = optional(string, "line") # for timeseries<br/>    precision    = optional(number, 2)      # for query_value<br/>    queries = list(object({<br/>      q            = string<br/>      display_type = optional(string, "line") # per-request override<br/>    }))<br/>  }))</pre> | `[]` | no |
