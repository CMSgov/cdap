CREATE VIEW bcda_prod_beneficiaries_attributed_to_active_entities AS
select sub.cms_id, sub.latest_cclf_file, cf.timestamp, COUNT(cb.id) as total_benes from (
	SELECT acos.cms_id as cms_id, MAX(cf.id) as latest_cclf_file FROM bcda_prod_active_acos acos
