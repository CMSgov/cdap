...
data "aws_iam_policy_document" "dpc_policies" {
  ...
  statement {
    ...
    resources = var.app == "dpc" && var.env == "test" ? [
      "arn:aws:rds-db:us-east-1:${data.aws_caller_identity.current.account_id}:dbuser:${data.aws_rds_cluster.this[0].cluster_resource_id}/${var.env}-dpc_attribution-role",
